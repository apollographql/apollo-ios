...
Hello! 👋 This short tutorial walks through the basic cocepts of code generation in Apollo iOS Client. It's intended to introduce you the core capabilities of code generation and will walk you
through the manual process to generate code using a test schema and a test operation. For more details on code generation please refer to the Code Generation documentation.

## Step 1: Setup

To start this tutorial first clone the [Apollo iOS project](https://github.com/apollographql/apollo-ios) from GitHub, in a Terminal run:

```bash
git clone https://github.com/apollographql/apollo-ios.git
```

## Step 2: Download a test GraphQL schema

 View the [StarWars GraphQL API](https://studio.apollographql.com/public/star-wars-swapi/home?variant=current) in Apollo Studio. We'll be using the [SDL public example](https://studio.apollographql.com/public/star-wars-swapi/schema/sdl?variant=current) as a test schema.

<img class="screenshot" src="../screenshot/studio-starwarsapi-download.png" alt="Where to download a schema from Apollo Studio" />

 On the right-hand side in Apollo Studio you can see the drop-down allowing downloads of either `JSON` or `Raw` SDL. For this tutorial select `Raw` which results in the downloading of a GraphQL Schema Definition file called: `star-wars-swapi@current.graphql`.
...
Now you are ready to generate some code!  Within the `codegen-test` directory in a Terminal run,

```bash
./apollo-ios-cli generate --path apollo-codegen-config.json
```

you should see a new file created in the `codegen-test` directory called `Query.graphql.swift` that looks like this:

<ExpansionPanel title="Swift Generated Code">
