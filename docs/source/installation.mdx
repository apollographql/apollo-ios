...
* Clone the [`xcode-apollo` repository](https://github.com/apollostack/xcode-apollo) to your computer.

* Close Xcode if it is currently running.

### Setup script

If you are running Xcode 11 or higher you can `cd` in to the `xcode-apollo` and run the following command: 

`sudo ./setup.sh`

### Manual installation

**Note**: You may need to `sudo` the following commands if are on `MacOS Catalina`
...
2. Copy `GraphQL.xclangspec` to `/Applications/Xcode.app/Contents/SharedFrameworks/SourceModel.framework/Versions/A/Resources/LanguageSpecifications`:
...
3. Copy `Xcode.SourceCodeLanguage.GraphQL.plist` to `/Applications/Xcode.app/Contents/SharedFrameworks/SourceModel.framework/Versions/A/Resources/LanguageMetadata` :
...
`
#### Xcode versions prior to 11

1. Copy `GraphQL.ideplugin` to `~/Library/Developer/Xcode/Plug-ins/`:

` cp -r GraphQL.ideplugin ~/Library/Developer/Xcode/Plug-ins/`

2. Copy `GraphQL.xclangspec` to `~/Library/Developer/Xcode/Specifications`:
