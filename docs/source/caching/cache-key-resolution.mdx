...
To instead configure cache key resolution based on a `union` type, use the union's [`possibleTypes`](apollographql.com/docs/ios/docc/documentation/apolloapi/union/possibletypes) property.
