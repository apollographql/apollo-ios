...
> You will need to have a clear understanding of how you are generating cache keys to be able to use this functionality effectively. To help you figure out how to construct a cache key or pattern to delete, we recommend having a clear understanding of [how cache keys are generated](./introduction#normalizing-responses) and [configuring custom cache key resolution](./cache-key-resolution) for your cache objects.

Removing objects by their cache key allows you to clear stale or unneeded cache data without deleting the entire cache.

In Apollo iOS, you can [configure custom cache keys for your objects](./cache-key-resolution). The key for an entry in the cache that uses custom cache keys will always have the format `${ObjectType}:${CustomCacheKey).

In this example, we've configured our cache to attempt to use the `id` field as a cache key.
...
This indicates that for every object which has an `id` property, it will be cached with that `id` as the key.
