...
> This section explains how cache keys are used to merge object data in the normalized cache. For information on how to configure your cache keys, see [Custom cache keys](./caching/cache-key-resolution).
...
> For more information on using `CacheKeyInfo` to configure cache keys, see [Custom cache keys](./caching/cache-key-resolution).
