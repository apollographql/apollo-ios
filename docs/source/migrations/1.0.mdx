...
- New generated models that improve readability and functionality while reducing generated code size
- New code generation tooling written in pure Swift code
- Support for using generated models in multi-module projects
- Type-safe APIs for cache key resolution

Because most of the library's core concepts are unchanged, the migration to 1.0 from the legacy 0.x versions of Apollo iOS shouldn't require much work. This page describes the most important changes, along with guidance on how to migrate an existing project from Apollo iOS 0.x to 1.0.

> **To start migrating to Apollo iOS 1.0:**
> - Read about the [key changes](#key-changes)
> - Follow the [step-by-step instructions](#step-by-step-instructions)
> - Use the [breaking changes](#breaking-changes) section to resolve remaining build errors
...
- Types for your  objects, interfaces, and unions 
...
To prevent naming conflicts, schema types are contained in their own namespace. You can generate this namespace as a stand-alone module, imported by your project, or as a caseless namespace enum that you can embed in your target application.

### Multi-module support

Apollo iOS 1.0 is designed to support both complex applications composed of multiple modules and monolithic application targets.

To support multi-module projects, the [generated schema module](#generated-schema-module) contains _all_ of your schema's shared types and metadata. This enables your generated operation models to be located anywhere in your project structure, as long as they are linked to the schema module.

The code generation engine provides flexible configuration options that aim to make code generation work seamlessly for any project structure.

## Step-by-step instructions

> **Before performing the migration to Apollo iOS 1.0, consider your project structure and decide how you'd like to include your generated schema module and operation models.**
>
> To learn how to configure Apollo iOS for your project structure, see [Project configuration](../project-configuration).

To migrate to Apollo iOS 1.0, you'll need to:
...
- [Refactor your code to use new APIs](#4-refactor-your-code)

Much of the migration process involves the new code generation mechanism, which is detailed in this step-by-step guide. 

As you go through this process, we'll explain how to remove deprecated pieces of the legacy 0.x version along the way. Each of the following steps also includes explanations for the breaking API changes in Apollo iOS 1.0.

### 1. Update to Apollo iOS 1.0

The first step is to update your Apollo iOS dependency to the latest version. You can include Apollo iOS as a package using Swift Package Manager (SPM) or Cocoapods.

In order to receive bug fixes and new features, we recommend including `1.0` up to the next major release.

> To see the modules provided by the Apollo iOS SDK, and determine which modules you need in your project, see [SDK components](./project-configuration#apollo-ios-sdk-components).
...
> **Note:** Apollo iOS 1.0 supports being built as a dynamic `.xcframework` or as a static library. You can build and include Apollo iOS 1.0 as a pre-compiled binary with a build tool such as Carthage or Buck, but we don't currently document this.

### 2. Set up code generation

Apollo iOS 1.0 includes a new code generation engine, written in 100% Swift code, replacing the legacy `apollo-tooling`. To use 1.0, you must install the new code generation engine and remove the old one.
...
Then, update the `ApolloCodegenConfiguration` in your script with the new configuration values. See [Codegen configuration](./../code-generation/codegen-configuration) for the list of configuration options.
