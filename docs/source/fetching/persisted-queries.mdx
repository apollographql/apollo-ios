...
> You can use _either_ Apollo Server or Apollo Router for APQs.
...
1. Generate and publish operation manifests (for persisted queries only)
...
An operation manifest acts as a safelist of trusted operations the [Apollo Router](/router/) can checks incoming requests against. You can generate operation manifests in two ways:
- via the [`generate`](./../code-generation/codegen-cli#generate) CLI command with `--path` and `--version` arguments
    - `path` is where to output the manifest file
    - `version` is either `persistedQueries` for persisted queries or `legacyAPQs` for APQs
- by adding the [`output.operationManifest`](./../code-generation/codegen-configuration#operation-manifest) option to your `ApolloCodegenConfiguration` JSON file
