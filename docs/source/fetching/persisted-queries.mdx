...
> This step is only required for implementing safelisting with persisted queries, it is _not_ for required APQs.

An operation manifest acts as a safelist of trusted operations the [Apollo Router](/router/) can check incoming requests against. You can generate operation manifests in two ways:
- by running the [`generate`](./../code-generation/codegen-cli#generate) or [`generate-operation-manifest`](./../code-generation/codegen-cli#generate-operation-manifest) CLI command with `--path` and `--version` arguments
    - `path` is where to write the manifest file
    - `version` is either `persistedQueries` for persisted queries or `legacyAPQs` for APQs
    - the [`generate-operation-manifest`](./../code-generation/codegen-cli#generate-operation-manifest) command generates the operation manifest without running full code generation
- by adding the [`output.operationManifest`](./../code-generation/codegen-configuration#operation-manifest) option to your `ApolloCodegenConfiguration` JSON file like so:
...
Setting the `output.operationManifest` option causes the operation manifest to be generated every time you run the `generate` command.
...
In the `options` section of your code generation configuration, set the `operationDocumentFormat` array to `definition`, `operationId`, or both `definition` and `operationId`. This determines how to generate the operation documents for your generated operations.
...
Once you've configured your code generation to include operation IDs, you can update your client to query by operation ID rather than the full operation string. This configuration is the same whether you are using APQ or persisted queries:
