...
The resulting operation manifest JSON looks something like this: 
...
> **Note:** This example uses the `persistedQueries` version of the operation manifest, the `legacyAPQs` version will look slightly different.
...
By default, the Apollo clients sends operation retries as `POST` requests.  In some cases, you may prefer or need to retry  persisted operation using a `GET` request—for example, your queries may make requests to a CDN that has better performance with `GET`s.
