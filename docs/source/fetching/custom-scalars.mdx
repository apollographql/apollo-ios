...
The type for your custom scalar must conform to the [`CustomScalarType`](https://www.apollographql.com/docs/ios/docc/documentation/apolloapi/customscalartype) protocol. This requires you to implement the `jsonValue` property and the `init()

For example, you could point the `typealias` for `UUID` to the `Swift.UUID` type
