...
However, remember that one of the advantages of GraphQL is that you can query for _exactly_ the data you need to display on a page. If you're not going to be displaying additional information, you can save bandwidth and execution time on both your frontend and backend by not asking for data until you need it.
...
This bit can be confusing, because GraphQL's assumptions about nullability are different from Swift's: 

* In Swift, if you annotate a property's type with either a question mark or an exclamation point, that property is nullable. 

* In GraphQL, if you annotate a field's type with an exclamation point, that field is *non-nullable*. This is becasue **GraphQL fields are nullable by default**. 
...
You may have noticed that the detail view includes a `Book Now!` button, but there's no way to book a seat yet. To fix that, let's [learn how to make changes to objects in your graph with mutations](tutorial_3).
