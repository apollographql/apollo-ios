...
In `LaunchesViewController.swift`, delete the following pieces of boilerplate code that the app doesn't need: 
...
Next, back in `LaunchesViewController.swift`, update the `prepareForSegue` method to obtain the most recently selected row and pass its corresponding launch details to the detail view controller: 
...
Let's take advantage of one of the built-in styles for `UITableViewCell` to display more information without having to do much work. Go to `Main.storyboard` and select the **Launches Scene**, which includes the `TableView` and the default cell. 
...
When SDWebImage finishes installing, go back to `LaunchesViewController.swift` and add the following import to the top of the file:
