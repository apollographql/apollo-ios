...
Lastly, update the `init()` method to pass `cursor: nil` to `LaunchListQuery`:

```swift title="LaunchListViewModel.swift"
init() {
    Network.shared.apollo.fetch(query: LaunchListQuery(cursor: nil)) { result in// highlight-line
        switch result {
```
