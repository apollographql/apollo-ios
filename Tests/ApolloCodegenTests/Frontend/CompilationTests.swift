...
class CompilationTests: XCTestCase {
  ...
  func test__compile__givenSchemaSDL_queryWithDeferDirective_notInSchema_onInlineFragment_hasDirective_andIsDeferred() throws {
    schemaSDL = """
    type Query {
      allAnimals: [Animal!]
    }
    ...
    let selection = try XCTUnwrap(compilationResult.operations.first?.selectionSet.firstField(for: "allAnimals")?.selectionSet?.selections.first)
    guard case let .inlineFragment(fragment) = selection else {
      fail("Expected InlineFragment, got \(selection)!")
      return
    }

    expect(fragment.directives).to(equal(expectedDirectives))
    expect(fragment.isDeferred).to(beTrue())
  }

  func test__compile__givenSchemaSDL_queryWithDeferDirective_notInSchema_onFragmentSpread_hasDirective_andIsDeferred() throws {
    schemaSDL = """
    type Query {
      allAnimals: [Animal!]
    }
    ...
    let selection = try XCTUnwrap(compilationResult.operations.first?.selectionSet.firstField(for: "allAnimals")?.selectionSet?.selections.first)
    guard case let .fragmentSpread(fragment) = selection else {
      fail("Expected FragmentSpread, got \(selection)!")
      return
    }
