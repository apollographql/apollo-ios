...
class CLIExtractorTests: XCTestCase {
  ...
  func validateCLIIsExtractedWithRealSHASUM(file: StaticString = #filePath,
    ...
    XCTAssertTrue(FileManager.default.apollo.doesDirectoryExist(atPath: binaryFolderURL.path),
    ...
    XCTAssertTrue(FileManager.default.apollo.doesFileExist(atPath: binaryURL.path),
  ...
  func testExtractingZip() throws {
    ...
    let binaryFolderURL = CodegenTestHelper.binaryFolderURL()
    XCTAssertFalse(FileManager.default.apollo.doesDirectoryExist(atPath: binaryFolderURL.path))
  ...
  func testMissingSHASUMFileButCorrectZipFileCreatesSHASUMFile() throws {
    let shasumFileURL = CodegenTestHelper.shasumFileURL()
    try FileManager.default.apollo.deleteFile(atPath: shasumFileURL.path)
    
    XCTAssertFalse(FileManager.default.apollo.doesFileExist(atPath: shasumFileURL.path))
    
    let cliFolderURL = CodegenTestHelper.cliFolderURL()
    let zipFileURL = ApolloFilePathHelper.zipFileURL(fromCLIFolder: cliFolderURL)
    
    XCTAssertTrue(FileManager.default.apollo.doesFileExist(atPath: zipFileURL.path))
    
    let binaryURL = try CLIExtractor.extractCLIIfNeeded(from: cliFolderURL)
    
    /// Was the binary extracted?
    XCTAssertTrue(FileManager.default.apollo.doesDirectoryExist(atPath: binaryURL.path))
    
    /// Did the SHASUM file get created?
    XCTAssertTrue(FileManager.default.apollo.doesFileExist(atPath: shasumFileURL.path))
    self.validateCLIIsExtractedWithRealSHASUM()
  }
}
