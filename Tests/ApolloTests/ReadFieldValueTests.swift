...
func readFieldValue(_ field: GraphQLField, from object: JSONObject) throws -> Any? {
  ...
  return try executor.execute(selections: [field], on: object, firstModifiedAt: Date(), withKey: "", variables: [:], accumulator: GraphQLSelectionSetMapper<MockSelectionSet>()).await().resultMap[field.responseKey]!
}
