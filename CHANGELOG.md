...
### Fixed
- **Avoid referencing pruned schema types in test mock defaults ([#1068](https://github.com/apollographql/apollo-ios-dev/pull/1068)):** With `reduceGeneratedSchemaTypes` enabled, the default value generated for a non-null interface or union field in a test mock convenience initializer always used the first implementing type in the schema, even when that type had been pruned from the generated output — emitting a reference to a `Mock` type that was never generated. Defaults now select the first implementing type that is actually generated, and when every implementing type has been pruned the parameter remains non-optional with no default value. Interface and union defaults are also now rendered through the configured type name, so custom type names and capitalization rules are respected instead of the raw schema name. _Thank you to [@karim-alweheshy](https://github.com/karim-alweheshy) for the contribution._
