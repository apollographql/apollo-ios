...
interface GraphQLListValue {
  kind: "ListValue";
  value: GraphQLValue[];
}

interface GraphQLObjectValue {
  kind: "ObjectValue";
  value: { [name: string]: GraphQLValue };
}

function valueFromValueNode(valueNode: ValueNode): GraphQLValue {
...
class Compiler {
  options: CompilerOptions;
  schema: GraphQLSchema;
  typesUsedSet: Set<GraphQLNamedType>;
  ...
  addTypeUsed(type: GraphQLType) {
    if (isNamedType(type) && this.typesUsedSet.has(type)) return;
  ...
  get typesUsed(): GraphQLNamedType[] {
