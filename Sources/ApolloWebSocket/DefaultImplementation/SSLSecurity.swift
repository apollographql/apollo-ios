...
open class SSLSecurity : SSLTrustValidator {
      ...
      DispatchQueue.global(qos: .default).async {
        let pubKeys = certs.reduce([SecKey]()) { (pubKeys: [SecKey], cert: SSLCert) -> [SecKey] in
          ...
          if let data = cert.certData, cert.key == nil {
            cert.key = SSLSecurity.extractPublicKey(data)
          }
          if let key = cert.key {
            pubKeys.append(key)
          }
  ...
  open func isValid(_ trust: SecTrust, domain: String?) -> Bool {
    ...
    if self.usePublicKeys {
      ...
      let shouldTrust = SecTrustEvaluateWithError(trust, &error)
      if shouldTrust && error == nil {
   ...
   */
  static public func extractPublicKey(_ data: Data) -> SecKey? {
  ...
  /**
   ...
   - returns: a public key
   */
  static public func extractPublicKey(_ cert: SecCertificate, policy: SecPolicy) -> SecKey? {
  ...
  public func publicKeyChain(_ trust: SecTrust) -> [SecKey] {
    ...
    let keys = (0..<SecTrustGetCertificateCount(trust)).reduce([SecKey]()) { (keys: [SecKey], index: Int) -> [SecKey] in
      var keys = keys
      let cert = SecTrustGetCertificateAtIndex(trust, index)
      if let key = SSLSecurity.extractPublicKey(cert!, policy: policy) {
        keys.append(key)
      }
