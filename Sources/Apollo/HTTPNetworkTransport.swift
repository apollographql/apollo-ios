...
public class HTTPNetworkTransport: NetworkTransport {
  ...
  private func request<Operation: GraphQLOperation>(for operation: Operation) -> URLRequest {
    ...
    if let uploadable = operation as? GraphQLUploadable {
      ...
      //Map
      let map = try! serializationFormat.serialize(value: uploadable.map)
      formData.append(map, withName: "map")
      
      //Files
      for (index, upload) in uploadable.uploads.enumerated() {
        let name = "\(index)"
        switch upload {
